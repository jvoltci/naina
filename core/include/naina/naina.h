...
typedef struct { float x, y, w, h, score; } naina_bbox;
typedef struct { float x, y; }              naina_point;
...
naina_status naina_init(const naina_config* cfg, naina_ctx_t** out_ctx);
void         naina_release(naina_ctx_t* ctx);
const char*  naina_version_string(void);
const char*  naina_status_str(naina_status s);

/* ─── Image wrapping (zero-copy view; lib does not own pixels) ───── */

naina_status naina_image_wrap(const uint8_t* data,
                              int32_t width, int32_t height, int32_t stride,
                              naina_pixfmt fmt,
                              naina_image_t** out_image);
void         naina_image_release(naina_image_t* image);

/* ─── Face stack ──────────────────────────────────────────────────── */

/* Detect: lib allocates output array, caller frees via naina_free_faces. */
naina_status naina_face_detect(naina_ctx_t* ctx, const naina_image_t* image,
                               naina_face** out_faces, int32_t* out_count);
void         naina_free_faces(naina_face* faces, int32_t count);

/* Embed: caller provides buffer of size naina_face_embed_dim() floats. */
int32_t      naina_face_embed_dim(const naina_ctx_t* ctx);
naina_status naina_face_embed(naina_ctx_t* ctx, const naina_image_t* image,
                              const naina_face* face, float* out_embedding);

/* Liveness: out in [0,1], higher = more live. Threshold per use case. */
naina_status naina_face_liveness(naina_ctx_t* ctx, const naina_image_t* image,
                                 const naina_face* face, float* out_score);

/* Pure-function similarity (cosine). Exposed for convenience. */
float        naina_embed_similarity(const float* a, const float* b, int32_t dim);

/* ─── Person stack ────────────────────────────────────────────────── */

naina_status naina_person_detect(naina_ctx_t* ctx, const naina_image_t* image,
                                 naina_person** out_persons, int32_t* out_count);
void         naina_free_persons(naina_person* persons, int32_t count);

int32_t      naina_reid_embed_dim(const naina_ctx_t* ctx);
naina_status naina_person_reid(naina_ctx_t* ctx, const naina_image_t* image,
                               const naina_person* person, float* out_embedding);

/* ─── Tracking (stateful, per video stream) ───────────────────────── */

naina_status naina_tracker_create(naina_ctx_t* ctx, naina_tracker_t** out);
void         naina_tracker_release(naina_tracker_t* tracker);

/* Feed per-frame detections; receive same boxes annotated with track_ids. */
naina_status naina_tracker_update(naina_tracker_t* tracker,
                                  const naina_person* detections, int32_t det_count,
                                  naina_person** out_tracked, int32_t* out_count);
